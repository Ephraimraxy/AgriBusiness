...
export const getResourcePersonRegistrations = async (): Promise<ResourcePerson[]> => {
  try {
    ...
    // Try resourcePersons collection (without underscore)
    console.log('Trying collection: resourcePersons');
    try {
      const resourcePersons = await getAllDocuments<ResourcePerson>("resourcePersons");
      console.log('Raw resource persons data from resourcePersons:', resourcePersons);
      if (resourcePersons.length > 0) {
        return resourcePersons;
      }
    } catch (error) {
      console.log('resourcePersons collection error:', error);
    }
...
export const getPublishedEvaluationQuestions = async (): Promise<EvaluationQuestion[]> => {
  try {
    const q = query(
      collection(db, EVALUATION_QUESTIONS_COLLECTION),
